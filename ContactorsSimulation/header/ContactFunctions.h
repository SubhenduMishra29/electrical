...
void autoAddMainContacts(Contactor& contactor, int numContacts) {
    for (int i = 1; i <= numContacts; ++i) {
        std::stringstream ss;
        ss << "MainContact " << i;
        contactor.addMainContact(i, ss.str(), i+100, "OutPort", 100.0); // Example parameters
    }
}
...
void autoAddAuxiliaryContacts(Contactor& contactor, int numContacts) {
    for (int i = 1; i <= numContacts; ++i) {
        std::stringstream ss;
        ss << "AuxContact " << i;
        contactor.addAuxiliaryContact(i, ss.str(), i+200, "OutPort", "NC", 50.0); // Example parameters
    }
}
